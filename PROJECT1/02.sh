...
ssh 192.168.2.11
sed -i '35c define('DB_HOST', '192.168.2.21');'  /usr/local/nginx/html/wp-config.php 
exit
#Web服务器集群
#步骤一：部署web2和web3服务器
for i in 12 13
do
sh-copy-id 192.168.2.$i
cp ./linux-soft/02/lnmp_soft.tar.gz  root@192.168.2.$i:
done
for i in 12 13
 do
...
nginx -s reload;
scp /usr/local/nginx/conf/nginx.conf root@192.168.2.13:/usr/local/nginx/conf/nginx.conf;"
nginx -s reload;
exit
#3）启动相关服务
for i in 12 13
